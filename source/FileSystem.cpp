...
vector<string> FileSystem::FindInFolders(string type, string path, bool relativeToCWD, string filemask) {
	...
	NuLogger::getInstance()->log("Search for Directories in: '%s'", sSearchPath.c_str());

	if ((hFind = FindFirstFile(sDir, &fdFile)) == INVALID_HANDLE_VALUE)
	{
		NuLogger::getInstance()->log("Path not found: [%s]", ws2s(sDir).c_str());
		return ret;
	}
	do
	{
			...
			&& wcscmp(fdFile.cFileName, L"..") != 0)
		{
			if (fdFile.dwFileAttributes &FILE_ATTRIBUTE_DIRECTORY)
			{
				if (type == "folder") {
					...
					// NuLogger::getInstance()->log("Directory found: '%s'", dirName.c_str());
				...
				if (type == "file") {
					...
					// NuLogger::getInstance()->log("File found: '%s'", fileName.c_str());
